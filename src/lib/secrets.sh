...
_encrypt_filename ()
{
    local encryption_key_cmd="printf '$1' | spectre -q -n -s 0 -F n -t n -u '$1' 'file_encryption_key'"
    encryption_key="$(qvm-run --pass-io "$VAULT_VM" "$encryption_key_cmd")"

    local encrypted_identity_command="printf '$encryption_key' | spectre -q -n -s 0 -F n -t n -u '$IDENTITY' '$1'"
